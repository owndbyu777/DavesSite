...
function buildXMLCompilerPage() {
    if (!divContainer) {
        ...
        var btnLoadXML = D('button', '', '', 'Load XML', { 'type': 'button' });
        btnLoadXML.onclick = function () {
            DavesSite.XMLCompiler.XMLCompiler.loadXML(JSON.stringify({ filePath: document.getElementById('txbFilePath').value }), function (rsp) {
                if (!rsp.error) {
        ...
        btnLoadXML.appendTo(divContainer);

        var btnSaveXML = D('button', '', '', 'Save as XML', { 'type': 'button' });
        btnSaveXML.onclick = function () {
            var obj = tblXML.buildSaveObj();
            obj.filePath = document.getElementById('txbFilePath').value;
            DavesSite.XMLCompiler.XMLCompiler.saveXML(JSON.stringify(obj), function (rsp) {
                if (!rsp.error) {
                    var objRsp = JSON.parse(rsp.value);
                    if (objRsp.success) {
                    } else {
        ...
        btnSaveXML.appendTo(divContainer);

        var btnSaveJSON = D('button', '', '', 'Save as JSON', { 'type': 'button' });
        btnSaveJSON.onclick = function () {
            var obj = tblXML.buildSaveObj();
            var filePath = document.getElementById('txbFilePath').value;
            DavesSite.XMLCompiler.XMLCompiler.saveJSON(JSON.stringify(obj), filePath, function (rsp) {
                if (!rsp.error) {
                    var objRsp = JSON.parse(rsp.value);
                    if (objRsp.success) {
                    } else {
        ...
        btnSaveJSON.appendTo(divContainer);
